package com.neohelden.universal_mic_stream
...
/** UniversalMicStreamPlugin */
class UniversalMicStreamPlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    channel = MethodChannel(flutterPluginBinding.flutterEngine.dartExecutor, "neohelden.com/universal_mic_stream")
  ...
  companion object {
    ...
    fun registerWith(registrar: Registrar) {
      val plugin = UniversalMicStreamPlugin()

      val channel = MethodChannel(registrar.messenger(), "neohelden.com/universal_mic_stream")
  ...
  private fun startAudioRecorder() {
    if (recorder == null) {
      Log.d("UniversalMicStreamPlugin", "Start audio recorder")
      recorder = AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate, channels, audioFormat, bufferSize)
      recorder?.startRecording()
    } else {
      Log.w("UniversalMicStreamPlugin", "Audio recorder does already exist. Care the recorder could be canceled somewhere else.")
    }
  }

  private fun stopRecording() {
    if (recorder != null) {
      Log.i("UniversalMicStreamPlugin", "Stopping audio recorder")
      isRecording = false
      recorder?.stop()
      recorder?.release()
      recorder = null
    } else {
      Log.i("UniversalMicStreamPlugin", "No audio recorder to stop")
    }
  }
