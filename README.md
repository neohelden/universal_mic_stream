...
So how to use **mic_stream_plus**? Actually it's pretty easy, just start it by calling the static method `MicStreamPlus.getMicStream()` and stop it with `MicStreamPlus.stopRecording()`**.** 

```dart
var stream = await MicStreamPlus.getMicStream();
